...
Map::Map() : speed(1.2), damage(0), enemy_count(0), map(NULL), dist(NULL), sprites(std::vector<Sprite>()), doors(std::vector<Door>())
{
	...
	//Обработка ошибок для загрузки текстуры
    if(!map_tex)
    {
    ...
    {
        std::cerr << "Map must be using 8bmp format." << std::endl;
        return;
    }

    //временный обходной путь, потому что размеры карты, кроме 32x32, вызывают сбои
	...
	for(ushort y = 0; y < h; y++)
	{
		for(ushort x = 0; x < w; x++)
		{
			...
			if(pixel == 0) //black 0,0,0 : трава
				map[id] = '0';
			else if(pixel == 65280) //green 0,255,0 : кирпич
				map[id] = '1';
			else if(pixel == 255) //blue 0,0,255 : булыжник
				map[id] = '2';
			else if(pixel == 16776960) //yellow 255,255,0 : дверь
			{
				map[id] = '3';
			...
			else if(pixel == 16711680) //red 255,0,0 : враг
			{
				map[id] = ' ';
			...
			else if(pixel == 65535) //cyan 0,255,255 : цветы
			{
				map[id] = ' ';
			...
			else if(pixel == 16711935) //magenta 255,0,255 : ключ
			{
				map[id] = ' ';
...
//возвращает true, если игрок открыл дверь
...
{
	//инициализировать карту расстояний
	for(int i = 0; i < h; i++)
	{
		for(int j = 0; j < w; j++)
		{
	...
	dist[px+py*w] = 0; //устанавливает расстояние игрока до 0
