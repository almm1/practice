...
int main()
{
    ...
    if(!renderer.init_sdl("ZOMBIE ASK", 1280, 720))
        return 0;
    ...
    while (!menu.wants_to_quit)
    {
        ...
        if(dt == 0) dt = 1; //предотвращаем деление на ноль

        //вызывается пять раз в секунду
        if(lastFPSDisplay + 200 < lastTime)
        {
            lastFPSDisplay = lastTime;
            fps = 1 / dt;

            //сортировать спрайты только 5 раз в секунду
        ...
        if(menu.current == None) //меню не отображается, проигрыватель в данный момент играет
        {
