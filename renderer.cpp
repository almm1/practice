...
void Renderer::draw(uint fps)
{
    ...
    for(int i = 0; i < screen_w; i++) //для каждого вертикального столбца пикселя
    {
        // угол луча в рад
        ...
        //нам не нужно вычислять sin and cos для каждого шага
        float x_offset = cos(ray_angle);
        float y_offset = sin(ray_angle);

        while(!hit_wall && dist < 40) //заставляет луч двигаться вперед шаг за шагом
        {
            dist += 0.02;
            ...
            bool valid = false;
            if(map->get_tile(ushort(ray_x), ushort(ray_y)) == '3') //дверь
            {
                ...
                valid = map->get_tile(ushort(ray_x), ushort(ray_y)) != ' '; //текущая плитка не пуста, мы врезались в стену
            ...
            if(valid)
            {
                ...
                //нам нужно спроецировать расстояние на плоскую плоскость, перпендикулярную игроку
                //чтобы избежать эффекта «рыбий глаз»
                projectDist = dist * cos(ray_angle - player->get_angle());
                //высота текущего вертикального сегмента для рисования
                ...
                // x-texcoord, нам нужно определить, попадаем ли мы в «вертикальную» или «горизонтальную» стену (по карте)
                ...
                if (tile_id == 2)
                    wall_top += (wall_bottom - wall_top) * 0.1;

                for(int j = 0; j < screen_h; j++)
                {
                    if(j < wall_top) //Рисуем небо (верх : 0,50,200 ; низ : 150,200,200)
                        set_pixel(i, j, rgb_to_int(150 * (j / (float)screen_h * 2), 50 + 150 * (j / (float)screen_h * 2), 300));
                    else if(j >= wall_bottom) //рисуем землю (верх : 0,100,0 ; низ : 100,200,0)
                    {
                        //отображает пиксель и применяет освещение
                        set_pixel(i, j, rgb_to_int(120 - 70 * ((screen_h - j) / (float)screen_h * 2), 120 - 50 * ((screen_h - j) / (float)screen_h * 2), 5));
                    }
                    else //верх < j < низ, рисуем стену
                    {
                        int texture_y = (j - wall_top) / (float)wall_height * wall_textures->h;
                        //отображает пиксель и применяет освещение
                        set_pixel(i, j, apply_light(get_pixel_tex(tile_id, texture_x, texture_y), vertical ? 1.2 : 0.7)); //чем больше цифра, тем больше контраста и света
                    }
                }
            }   
        }
    ...
    if(menu->current == None)
    {
       ...
       draw_2d_sprite(0, offset, screen_h - 400, 400.0); //рисует оружие, когда ходит игрок

        // рисует прицел
        int center = screen_w / 2;
        middle += 30;
        ushort crosshair_size = 8;
        Uint32 crosshair_color = rgb_to_int(255, 255, 255);
        ...
        for(int i = screen_h - 58; i < screen_h - 8; i++)
        {
            ...
            {
                set_pixel(j, i, j < hp_width ? color : bg_color); //теряет здоровье = теяется отображение линии
            }
        }
...
void Renderer::draw_sprite(Sprite s)
{
    ...
    // проверьте, находится ли спрайт в конусе просмотра (типа как конусом от наблюдателя идут лучи)
    if(abs(sprite_dir - player->get_angle()) > fov / 2.0)
        return;

    //вычисляет размер спрайта и смещение в пространстве экрана
    ...
    //вычисляет коробку спрайта, чтобы потом было легче вычислить
    int middle = screen_h / 2;
...
{   
    ...
    std::string score_text = std::to_string(map->enemy_count) + " Enemys  left - Timer:" + menu->timer.get_time_string();
...
{   
    draw_text(300, 50, "ZOMBIE ASK", true, ttf_color_red); //левый отступ, отступ вниз
...
{   
    ...
    draw_text(20, 450, "AL LIVES MATTERS, but ...", false, ttf_color_white);
    draw_text(20, 520, "try to kill all zombies!", false, ttf_color_white);
